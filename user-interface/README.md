...
The signature is used to verify that the sender of the JWT is who it says it is and to ensure that the inputSTOMPMessage wasn't changed along the way.
...
7. Make a GET request to `/users/me` to check you're not authenticated. You should receive a response with a `403` with an `Access Denied` inputSTOMPMessage since you haven't set your valid JWT token yet
